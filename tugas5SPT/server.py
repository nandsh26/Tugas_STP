...
while True:
    conn, addr = server.accept() # menerima koneksi client
    clients.append(conn) # menambahkan client ke daftar
    thread = threading.Thread(target=handle_client, args=(conn, addr, clients)) # membuat thread baru untuk client
    thread.start() 